    ...
    if (name.find("AmpAllChannels") != std::string::npos) {
      ...
      if (!isHistogramReasonable(hist, name)) {
        ...
        if (result == Quality::Good)
          result = Quality::Medium; // Less critical than AmpAllChannels
      } else {
        mResults.histogramsWithSufficientStats++;
      }
      updateResultsFromHistogram(hist, name);
...
Quality AmplitudeDistributionCheck::analyzeIndividualChannels(std::map<std::string, std::shared_ptr<MonitorObject>>* moMap)
{
  ...
  for (auto& [moName, mo] : *moMap) {
    ...
    if (channelsAnalyzed >= mMaxChannelsToAnalyze)
      break;
...
void AmplitudeDistributionCheck::updateResultsFromHistogram(TH1* hist, const std::string& name)
{
  ...
  if (mean < mResults.minChannelMean)
    mResults.minChannelMean = mean;
  if (mean > mResults.maxChannelMean)
    mResults.maxChannelMean = mean;
  ...
  if (hist->GetEntries() > 0) {
    ...
    if (overflowFraction > mMaxOverflowFraction)
      mResults.channelsWithOverflow++;
    if (underflowFraction > mMaxUnderflowFraction)
      mResults.channelsWithUnderflow++;
  }
}
